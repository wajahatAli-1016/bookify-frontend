      ...
      >
         ...
         news.map((item,index)=>(
            <SwiperSlide key={index} style={{width:'500px', height:'270px', paddingLeft:'50px', paddingRight:'55px'}} >
             <div className='news' style={{display:'flex', flexDirection:'row', alignItems:'center'}}>
                <div style={{padding:'10 10 10 10'}}>
                   <Link to="/">
                   <h3 className='text-lg font-medium hover:text-blue-500 mb-4'>{item.title}</h3>
                   </Link> 
                   <div className='w-12 h-[4px] mb-5' style={{backgroundColor:'#FFCE1A'}}></div>
                   <p className='text-sm text-gray-600'>{item.description}</p>
                </div>

                <div style={{width:'200px',height:'250px'}}>
                    <img src={item.image} className ='w-full object-cover' style={{marginTop:'70px'}} alt="" />
                </div>
             </div>
            </SwiperSlide>
         ))
